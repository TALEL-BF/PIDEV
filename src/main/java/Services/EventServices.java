...
import com.auticare.iservices.IEventServices;
...
public class EventServices implements IEventServices {
    ...
    // ================= ADD EVENT (SANS PLANNING) =================
    @Override
    public void ajoutEvent(Event event) {
        String req = "INSERT INTO event (titre, description, typeEvent, lieu, maxParticipant, dateDebut, dateFin, heureDebut, heureFin, image, planning) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try {
            ...
            ps.setString(11, event.getPlanning()); // ← AJOUT DU PLANNING
    ...
    // ================= UPDATE EVENT (AVEC PLANNING) =================
    @Override
    public void modifierEvent(Event event) {
        String req = "UPDATE event SET titre=?, description=?, typeEvent=?, lieu=?, maxParticipant=?, dateDebut=?, dateFin=?, heureDebut=?, heureFin=?, image=?, planning=? WHERE idEvent=?";
    ...
    // ================= DISPLAY ALL EVENTS (AVEC PLANNING) =================
    @Override
        ...
        try {
            ...
            while (rs.next()) {
                Event event = new Event(
                        ...
                        rs.getTime("heureFin"),
                        rs.getString("image"),
                        rs.getString("planning") // ← AJOUT DU PLANNING
                );
                events.add(event);
            }
        } catch (SQLException e) {
    ...
    // ================= SUPPRIMER LE PLANNING D'UN ÉVÉNEMENT =================
    public void supprimerPlanning(int idEvent) {
        String req = "UPDATE event SET planning = NULL WHERE idEvent = ?";

        try {
            PreparedStatement ps = con.prepareStatement(req);
            ps.setInt(1, idEvent);
            ps.executeUpdate();
            System.out.println("✅ Planning supprimé pour l'événement ID: " + idEvent);
        } catch (SQLException e) {
            System.err.println("❌ Erreur suppression planning: " + e.getMessage());
            e.printStackTrace();
        }
    }
    // Dans EventServices.java
    // Dans EventServices.java
    @Override
